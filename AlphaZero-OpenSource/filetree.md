...
待改2：我才试了一下，就是根据readme，然后有几个userwarning，你看看什么问题，一并改了
...
待改4：我觉得训练啊，需要个独立的config.py（我倾向于config.py）或者config.yaml其他的之类的配置文件，
训练配置文件不至于这些内容
board:
  width: 6
  height: 6
...
待改5：我觉得训练是不是要有个log日志啊，其实是终端输出把文本保存下来就行了吧（可能是），终端输出是给开发者实时看的。
...
待改6：训练过程可视化工具可不可以搞一搞？折线图之类的，存为文件。


待改7：也许，文件树是不是该改改了
待改8：我看不懂现有代码啊，所以你生成的新的代码，要完整，还要有适当的中文注释
待改9：对新的项目文件体系，写个readme_zh.md嘛，介绍文件树（结构），文件功能，文件互联作用流程，快速开始之类的

以上为9个点。
请把重建后的项目文件树贴出来，每个文件内的完整代码给我，最后给个readme