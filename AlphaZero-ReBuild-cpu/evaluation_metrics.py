...
class EvaluationMetrics:
    ...
    def record_game(self, winner, moves, advantage_changes):
        ...
        self.advantage_history.append({
            'avg': np.mean(advantage_changes) if advantage_changes else 0,
            'std': np.std(advantage_changes) if advantage_changes else 0,
    ...
    def get_metrics(self):
        ...
        win_ratio = (self.win_counts[1] + 0.5 * self.win_counts[-1]) / total_games
        
        return {
            'win_ratio': win_ratio,
            'avg_moves': np.mean(self.move_data) if self.move_data else 0,
            'advantage_std': np.mean([x['std'] for x in self.advantage_history]) if self.advantage_history else 0,
            'max_blunder': np.max([x['max_blunder'] for x in self.advantage_history]) if self.advantage_history else 0,
            'turning_points': np.mean([x['turning_points'] for x in self.advantage_history]) if self.advantage_history else 0,
            'matchup_stats': dict(self.win_counts)
        }