...
class Trainer:
    def __init__(self, model, config):
        ...
        # 新增：启用 torch.compile 加速（如果支持）
        if hasattr(torch, 'compile'):
            self.model = torch.compile(self.model)
            logger.info("已启用 torch.compile 加速模型推理")
        ...
        # 新增：混合精度训练支持
        # ✅ 新增：混合精度训练支持（非常重要）
        self.scaler = torch.amp.GradScaler("cuda") if config.use_gpu and torch.cuda.is_available() else None
    ...
    def train(self):
        ...
        try:
            ...
            # 替换原来的 backward 流程
            with autocast(enabled=config.use_gpu):
